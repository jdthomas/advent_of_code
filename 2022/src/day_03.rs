...
pub fn solve_problem_1(lines: &[String]) -> Result<i64> {
    // println!("here: {:?}", lines);
    let result: i64 = lines.iter().map(|line| {
...
pub fn solve_problem_2(lines: &[String]) -> Result<i64> {
    let res = lines.iter().chunks(3).into_iter().map(|elf_group| {
        let mut elf_group = elf_group.into_iter().map(|line| line.chars().collect::<HashSet<char>>());
